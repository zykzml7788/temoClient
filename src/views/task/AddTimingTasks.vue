<template>
  <el-dialog title="新增定时任务" :visible.sync="$store.state.addtimingtaskshow" style="height: 100%;" :close-on-click-modal="false" :append-to-body="true"
             @close="closeAddTimingTaskView">
    ...
    <div slot="footer" class="dialog-footer">
      <el-button type="primary" @click="addTimingTask">保 存</el-button>
      <el-button @click="closeAddTimingTaskView">取 消</el-button>
    </div>
  </el-dialog>
</template>
        ...
        methods:{
            addTimingTask(){
                ...
                this.$refs['form'].validate(bol=>{
                    if (bol){
                        this.$axios.post('/apis/timingTask/',task).then(res=>{
            ...
            closeAddTimingTaskView(){
                ...
                this.$store.commit('changeAddTimingTaskShow',false);
            },
            showAddTestSet(){
